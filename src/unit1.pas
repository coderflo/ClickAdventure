...
begin
  ...
  gender := QuestionDlg('Vorbereitung','Wähle ein Geschlecht',mtCustom,[1,'männlich','IsDefault', 2,'weiblich'],'');
  ...
  rooms[44].descriptionBeforeRiddle:=compile('Svaard: Tretet vor. %n: Ich habe Issormir geschlachtet. Hier ist sein Kopf. Svaard: Schweigt. Die Macht der Erhabenen glüht in Euch. Ihr müsst sterben. Für Kralkatorrik.');
  ...
  rooms[45].descriptionBeforeRiddle:=compile('%n: Das Totem ist in Flammen aufgegangen. Doch nichts ist passiert. Ich sollte besser hier verschwinden bevor… Gaheron Baelfeuer: Ich kenne Euch nicht, Eindringling. Doch die anderen, die es versucht haben, habt Ihr gerade verbrannt auf dem Boden liegen gesehen. Mir egal. Eine Mahlzeit mehr. %n: Wartet! Ich will Euch nichts tun, Gaheron! Ich habe nur ein paar Fragen. Was könnt Ihr mir über die Erhabenen und Kralkatorrik sagen? Ist das alles nur ein Gerücht? Gaheron Baelfeuer: Die Erhabenen müssen vernichtet werden. Kralkatorriks Diener werden sein Werk vollbringen. %n: (Mein Kopf tut höllisch weh, als würde jemand versuchen, darauf zuzugreifen!) Gaheron Baelfeuer: Erhabener, hier ist die Sackgasse Eurer Reise!');
  ...
  rooms[46].descriptionBeforeRiddle:=compile('%n: Zeigt Euch! Wer seid ihr?! KRALKATORRIK: ICH BIN ÜBERALL! IHR KÖNNT MICH NICHT BEZWINGEN! IHR ENDET GENAU SO, WIE ES DIE ANDEREN ERHABENEN GETAN HABEN! %n: Ich bin kein Erhabener! Lasst mich gehen! Warum bin ich hier?! KRALKATORRIK: IHR HATTET DIE MACHT, MEINE CHAMPIONS SVAARD UND GAHERON ZU BESIEGEN. NUR EIN ERHABENER BESITZT DIESE MACHT. LASST MICH EUCH AUS DEM WEG RÄUMEN. %n: (Es hat keinen Sinn. Ich muss wohl kämpfen. Bringt es zu Ende! Gaherons Zepter wird mir helfen.) ');
  // room 47 (34)
  ...
  rooms[47].descriptionBeforeRiddle:=compile('%n: Zeigt Euch! Wer seid ihr?! KRALKATORRIK: ICH BIN ÜBERALL! IHR KÖNNT MICH NICHT BEZWINGEN! IHR ENDET GENAU SO, WIE ES DIE ANDEREN ERHABENEN GETAN HABEN! %n: Ich bin kein Erhabener! Lasst mich gehen! Warum bin ich hier?! KRALKATORRIK: IHR HATTET DIE MACHT, MEINE CHAMPIONS SVAARD UND GAHERON ZU BESIEGEN. NUR EIN ERHABENER BESITZT DIESE MACHT. LASST MICH EUCH AUS DEM WEG RÄUMEN. %n: (Es hat keinen Sinn. Ich muss wohl kämpfen. Bringt es zu Ende! Gaherons Zepter wird mir helfen.) ');
  ...
  rooms[40].item:=TItem.create(12,'01010010 01100101 01100001 01100100 01101101 01100101','Ihr findet eine merkwürdige Inschrift: 01010010 01100101 01100001 01100100 01101101 01100101');
  ...
  rooms[48].descriptionBeforeRiddle:=compile('Stimme: Hier ist sie. Die Prüfungsstätte. Ihr werdet eine Legende, %n. %n: Ich werde es tun. Beginnt.');
  // room 49 (43)
  rooms[49].applyRiddle(riddlesEasy[category3][3]);
  rooms[49].labelSouth:='Nächstes Rätsel';
  rooms[49].south:=rooms[50];
  rooms[49].descriptionBeforeRiddle:=compile('Stimme: Hier ist sie. Die Prüfungsstätte. Ihr werdet eine Legende, %n. %n: Ich werde es tun. Beginnt.');
  // room 50 (43)
  rooms[50].applyRiddle(riddlesBoss[category1][3]);
  rooms[50].labelSouth:='Nächstes Rätsel';
  rooms[50].south:=rooms[51];
  rooms[50].descriptionBeforeRiddle:=compile('Stimme: Hier ist sie. Die Prüfungsstätte. Ihr werdet eine Legende, %n. %n: Ich werde es tun. Beginnt.');
  // room 51 (43)
  rooms[51].applyRiddle(riddlesBoss[category2][3]);
  rooms[51].labelSouth:='Nächstes Rätsel';
  rooms[51].south:=rooms[52];
  rooms[51].descriptionBeforeRiddle:=compile('Stimme: Hier ist sie. Die Prüfungsstätte. Ihr werdet eine Legende, %n. %n: Ich werde es tun. Beginnt.');
  // room 52 (43)
  rooms[52].applyRiddle(riddlesBoss[category3][3]);
  rooms[52].descriptionBeforeRiddle:=compile('Stimme: Hier ist sie. Die Prüfungsstätte. Ihr werdet eine Legende, %n. %n: Ich werde es tun. Beginnt.');
  ...
  rooms[44].respawn:=rooms[1];
  for i := 14 to 29 do
  begin
    rooms[i].respawn:=rooms[14];
  end;
  rooms[45].respawn:=rooms[14];
  for i := 30 to 34 do
  begin
    rooms[i].respawn:=rooms[30];
  end;
  rooms[46].respawn:=rooms[30];
  rooms[47].respawn:=rooms[30];
  for i := 35 to 43 do
  begin
  ...
  // update backgound images corresponding to gender
  if(gender = 1) then
  begin
    imgAppend:='.M.jpg';
  end
  ...
  begin
    ...
    if(rooms[i].backgroundImagePath <> '') then
    begin
      rooms[i].backgroundImagePath:=rooms[i].backgroundImagePath+imgAppend;
    end;
